...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance();            //initialize vulkan lib and connect to our application
  setupDebugMessenger();       //set up validation layers to debug errors
  createSurface();             //connects window and vulkans ability to display results
  pickPhysicalDevice();        //pick the physical device in the system (gpu)
  createLogicalDevice();       //describes what features of physical device we want to use
  createCommandPool();         //helps with command buffer allocation
}
